import { useState, useEffect } from "react";
import api from "../../api/axios";
import { FiBox, FiClock, FiInfo, FiLayers } from "react-icons/fi";

const StockLogs = () => {
  const [logs, setLogs] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetchStockLogs();
  }, []);

  const fetchStockLogs = async () => {
    try {
      const res = await api.get("/admin/stock-logs");
      setLogs(res.data);
    } catch (error) {
      console.error("Failed to fetch stock logs", error);
    } finally {
      setLoading(false);
    }
  };

  const getActionBadge = (action) => {
    if (action === "ORDER_CANCELLED" || action === "RETURN_COMPLETED") {
      return "bg-emerald-50 text-emerald-600 border-emerald-100";
    }
    if (action === "ORDER_PLACED") {
      return "bg-amber-50 text-amber-600 border-amber-100";
    }
    if (action === "ADMIN_ADJUSTMENT") {
      return "bg-indigo-50 text-indigo-600 border-indigo-100";
    }
    return "bg-gray-50 text-gray-700 border-gray-200";
  };

  if (loading) {
    return (
      <div className="flex justify-center items-center h-64 text-gray-500 font-medium space-x-3">
        <div className="w-6 h-6 border-4 border-indigo-200 border-t-indigo-600 rounded-full animate-spin"></div>
        <span>Loading stock logs...</span>
      </div>
    );
  }

  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight flex items-center gap-3">
            <FiLayers className="text-indigo-600" /> Inventory Stock Logs
          </h1>
          <p className="text-gray-500 font-medium mt-2 max-w-2xl">
            A real-time ledger of all stock quantity changes triggered by orders, cancellations, returns, or manual adjustments.
          </p>
        </div>
      </div>

      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        {logs.length === 0 ? (
          <div className="p-12 text-center text-gray-500 font-medium">
            <FiBox className="mx-auto text-4xl mb-4 text-gray-300" />
            No stock logs recorded yet.
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50/50 border-b border-gray-100">
                  <th className="py-4 px-6 text-xs font-bold text-gray-500 uppercase tracking-widest">
                    Timestamp
                  </th>
                  <th className="py-4 px-6 text-xs font-bold text-gray-500 uppercase tracking-widest">
                    Product
                  </th>
                  <th className="py-4 px-6 text-xs font-bold text-gray-500 uppercase tracking-widest">
                    Trigger Action
                  </th>
                  <th className="py-4 px-6 text-xs font-bold text-gray-500 uppercase tracking-widest text-center">
                    Change
                  </th>
                  <th className="py-4 px-6 text-xs font-bold text-gray-500 uppercase tracking-widest text-center">
                    Running Stock
                  </th>
                  <th className="py-4 px-6 text-xs font-bold text-gray-500 uppercase tracking-widest">
                    Order Ref
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {logs.map((log) => (
                  <tr key={log._id} className="hover:bg-gray-50/50 transition-colors">
                    
                    {/* Timestamp */}
                    <td className="py-5 px-6 whitespace-nowrap">
                      <div className="flex items-center gap-2 text-sm text-gray-600 font-medium">
                        <FiClock className="text-gray-400" />
                        {new Date(log.createdAt).toLocaleString(undefined, {
                          month: 'short', day: 'numeric',
                          hour: '2-digit', minute: '2-digit'
                        })}
                      </div>
                    </td>

                    {/* Product */}
                    <td className="py-5 px-6">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl overflow-hidden bg-gray-100 shrink-0 border border-gray-200">
                           {log.productId?.image ? (
                             <img src={log.productId.image} alt={log.productId.name} className="w-full h-full object-cover" />
                           ) : (
                             <div className="w-full h-full flex items-center justify-center text-gray-400"><FiBox /></div>
                           )}
                        </div>
                        <div>
                          <p className="text-sm font-bold text-gray-900 line-clamp-1">
                            {log.productId?.name || "Deleted Product"}
                          </p>
                          <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">
                            ID: {log.productId?._id?.slice(-6) || "N/A"}
                          </p>
                        </div>
                      </div>
                    </td>

                    {/* Action */}
                    <td className="py-5 px-6 whitespace-nowrap">
                      <span className={`px-3 py-1 text-xs font-bold rounded-full border ${getActionBadge(log.action)}`}>
                        {log.action.replace(/_/g, " ")}
                      </span>
                    </td>

                    {/* Change */}
                    <td className="py-5 px-6 whitespace-nowrap text-center">
                      <span className={`text-sm font-extrabold ${log.quantityChanged > 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                        {log.quantityChanged > 0 ? '+' : ''}{log.quantityChanged}
                      </span>
                    </td>

                    {/* Running Stock */}
                    <td className="py-5 px-6 whitespace-nowrap text-center">
                      <span className="text-sm font-bold text-gray-800 bg-gray-100 px-3 py-1 rounded-lg">
                         {log.updatedStock}
                      </span>
                    </td>

                    {/* Order Ref */}
                    <td className="py-5 px-6 whitespace-nowrap">
                      {log.orderId ? (
                        <p className="text-sm font-medium text-indigo-600 flex items-center gap-1.5">
                          <FiInfo className="text-indigo-400"/>
                          {log.orderId._id.slice(-8).toUpperCase()}
                        </p>
                      ) : (
                        <span className="text-sm text-gray-400 font-medium border border-dashed border-gray-300 px-2 py-0.5 rounded">N/A</span>
                      )}
                    </td>

                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        )}
      </div>
    </div>
  );
};

export default StockLogs;
